# -*- coding: utf-8 -*-
"""
Created on Sun Feb 16 19:15:52 2025
...
def ppl(datos_region, datos_deposito, K):
    # -------------------------------------------------------------
    # Recolección de arcos con servicio (regiones)
    # -------------------------------------------------------------
    c_sencillos = [arco
                   for data in datos_region.values()
    ...
    # Recolección de arcos sin servicio (depósito)
    ...
    # x_{i,j,k}, f_{i,j,k} y delta_{i,k}
    ...
    # -------------------------------------------------------------
    # Función objetivo: Minimizar la suma total de costos
    # -------------------------------------------------------------
    problema += lpSum(
    ...
    # -------------------------------------------------------------
    # Restricción de salida desde el depósito
    # -------------------------------------------------------------
    for k in range(1, K+1):
        out_0 = lpSum(
    ...
    # -------------------------------------------------------------
    # Restricción de equilibrio de flujo (in_x = out_x) para cada vehículo
    # -------------------------------------------------------------
    for k in range(1, K+1):
        for nodo in nodos:
            in_x  = lpSum(x.get((i, nodo, k), 0) for i in nodos if (i, nodo, k) in x)
            out_x = lpSum(x.get((nodo, j, k), 0) for j in nodos if (nodo, j, k) in x)
            problema += (in_x == out_x)

    # -------------------------------------------------------------
    # Restricciones de subtour: x_{i,j,k} <= M * delta_{(i,k)}
    # -------------------------------------------------------------
    M = n
    for i in nodos:
        for j in nodos:
    ...
    # -------------------------------------------------------------
    # Flujo auxiliar en cada vehículo
    # -------------------------------------------------------------
    for k in range(1, K+1):
    ...
    if estado == LpStatusOptimal:
        print("Valores de las variables x_{i,j,k}:")
        rutas_por_vehiculo = {k: [] for k in range(1, K+1)}
        
        for (i, j, k), var in x.items():
            valor = var.varValue
            if valor > 0:
                print(f"x_{i}_{j}_k{k} = {valor}")
...
if __name__ == "__main__":
    ...
    # Mostrar la ruta Euleriana y el costo de cada vehículo
    for vehiculo, lista_arcos in rutas_por_vehiculo.items():
        g = grafo(lista_arcos)
        euler_path = euler(g, deposito="0")

        # Calcular el costo sumando costo_arcos[(i,j)]
        costo_ruta = 0
